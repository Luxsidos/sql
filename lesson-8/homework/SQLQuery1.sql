...
-- 1
select Category, Count(*) as TotalNumber from Products
group by Category

-- 2
select Category, AVG(Price) as TotalPrice from Products
group by Category
...
-- 7
select ProductName, IIF(StockQuantity < 30, 'Low Stock', 'Sufficient') as StatusStock from Products

-- 8
select Country, COUNT(*) as TotalNumber from Customers
group by Country
...
select Year(OrderDate) as Year, sum(TotalAmount) as TotalAmount from Orders
